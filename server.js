// =============================================
// server.js - نسخه کامل با پشتیبانی از اسم‌ها
// =============================================
        ...
        try {
            ...
            switch (msg.type) {
                case 'auth':
                    ...
                    const character = msg.character || 'default';
                    const player_name = msg.player_name || player_id;
                    ...
                    log(`✅ ${player_id} (${player_name}) authenticated`, 'success');
                ...
                case 'friend_request':
                    ...
                    if (target && target.online) {
                        ...
                        log(`📩 Request from ${msg.player_name || msg.from_id} to ${msg.to_id}`, 'info');
                    } else {
                        ws.send(JSON.stringify({
                            type: 'error',
                ...
                case 'friend_request_accepted':
                    ...
                    if (fromPlayer && fromPlayer.online && toPlayer && toPlayer.online) {
                        ...
                        sendToPlayer(from_id, {
                            ...
                            player_name: toPlayer.player_name || to_id,
                            room_id: room_id,
                            timestamp: Date.now()
                        });
                        ...
                        sendToPlayer(to_id, {
                            ...
                            player_name: fromPlayer.player_name || from_id,
                            room_id: room_id,
                            timestamp: Date.now()
                        });
                        
                        log(`🎮 Room ${room_id} created between ${fromPlayer.player_name} and ${toPlayer.player_name}`, 'success');
                    }
                    break;
